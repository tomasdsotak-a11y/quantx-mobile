...
<head>
    ...
    <style>
        :root { --bg-main: #000; --bg-card: #0a0f1d; --border-card: #1c2538; --text-primary: #fff; --text-secondary: #86868b; --accent-blue: #0a84ff; --accent-green: #30d158; --accent-orange: #ff9f0a; --accent-red: #ff453a; }
        body { font-family: -apple-system, sans-serif; background: var(--bg-main); color: var(--text-primary); padding: 20px; margin: 0; }
        .category-ribbon { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin-bottom: 20px; }
        .cat-btn { background: #1c1c1e; border: 1px solid var(--border-card); color: var(--text-secondary); padding: 10px; border-radius: 8px; cursor: pointer; }
        ...
        select.native-select { width: 100%; padding: 10px; border-radius: 8px; background: #1c1c1e; color: #fff; border: 1px solid var(--border-card); }
        .card { background: var(--bg-card); border: 1px solid var(--border-card); border-radius: 14px; padding: 16px; margin-bottom: 16px; }
        .master-select { width: 100%; padding: 14px; border-radius: 10px; background: #1c1c1e; color: #fff; border: 1px solid var(--border-card); font-weight: 600; }
        ...
        .bet-slip-drawer { position: fixed; bottom: 0; left: 0; right: 0; background: rgba(20,20,22,0.96); padding: 12px 16px; border-top-left-radius: 20px; border-top-right-radius: 20px; z-index: 999; }
        .slip-item-card { background: #1c1c1e; padding: 10px; border-radius: 10px; font-size: 13px; }
    </style>
</head>
<body>
    ...
    <div class='filter-row'>
        <select class='native-select' id='drop-1-filter' onchange='applyFilters()'></select>
        <select class='native-select' id='drop-2-filter' onchange='applyFilters()'></select>
        <select class='native-select' id='drop-3-filter' onchange='applyFilters()'></select>
    </div>
    <div class='card'>
    ...
    <script>
        ...
        function switchAppScopeCategory(cat) {
            document.querySelectorAll('.cat-btn').forEach(b => b.classList.remove('active'));
            document.getElementById(`cat-${cat}`).classList.add('active');
            const matches = DB.filter(m => m.cat === cat);
            ...
            updateSelect('drop-1-filter', tiers, "All Groups");
            updateSelect('drop-2-filter', countries, "All Teams");
            updateSelect('drop-3-filter', dates, "All Dates");
            applyFilters();
        }
        ...
        function applyFilters() {
            const g = document.getElementById('drop-1-filter').value;
            const t = document.getElementById('drop-2-filter').value;
            const d = document.getElementById('drop-3-filter').value;
            const ms = document.getElementById('match-select');
            ms.innerHTML = '';
            const activeCat = document.querySelector('.cat-btn.active').id.replace('cat-', '');
            DB.filter(m => m.cat === activeCat).forEach(m => {
                if((g === 'all' || m.tier === g) && (t === 'all' || m.home === t || m.away === t) && (d === 'all' || m.date === d)) {
                    ms.innerHTML += `<option value="${m.id}">${m.home} vs ${m.away}</option>`;
                }
            });
        }
        switchAppScopeCategory("WC");
    </script>
</body>
</html>
"""
