    ...
    {% if report %}
        ...
        <div class='card'>
            ...
            <table class='matrix-table'>
                <thead>
                    <tr><th class='matrix-hdr' style='width: 50%;'>Statistical Parameter</th><th class='matrix-hdr' style='width: 25%;'>[ Base ]</th><th class='matrix-hdr' style='width: 25%; text-align: right;'>[ Behaved ]</th></tr>
                </thead>
                <tbody>
    ...
    <script>
        const GLOBAL_PYRAMID_CONTEXT_DATA = [
            {% for match in schedule %}
                { index: {{ loop.index0 }}, cat: "{{ match.cat }}", home: "{{ match.home }}", away: "{{ match.away }}", country: "{{ match.country }}", tier: "{{ match.tier }}", round: "{{ match.round }}", date: "{{ match.date }}", city: "{{ match.city }}" }{% if not loop.last %},{% endif %}
            {% endfor %}
        ];
        ...
        const dataPayloadContext = { homeTeam: "{% if report %}{{ report.h_name }}{% endif %}", awayTeam: "{% if report %}{{ report.a_name }}{% endif %}", hOdds: {% if report %}{{ report.behav_odds_0 }}{% else %}1.0{% endif %}, aOdds: {% if report %}{{ report.behav_odds_2 }}{% else %}1.0{% endif %}, dcHome: {% if report %}{{ report.behav_dc_0 }}{% else %}1.0{% endif %}, dcAway: {% if report %}{{ report.behav_dc_1 }}{% else %}1.0{% endif %}, avgCorners: {% if report %}{{ report.behav_corners }}{% else %}0.0{% endif %}, avgCards: {% if report %}{{ report.behav_cards }}{% else %}0.0{% endif %} };
        function returnToLandingScreen() { window.location.href = '/'; }
        function switchAppScopeCategory(categoryKey) {
            activeAppCategory = categoryKey; localStorage.setItem('fc_active_cat', categoryKey);
        ...
        function repopulateDynamicDropdownOptions(category, phase) {
            const d1 = document.getElementById('drop-1-filter'); const d2 = document.getElementById('drop-2-filter'); const d3 = document.getElementById('drop-3-filter');
            ...
            scopeMatches.forEach(m => {
                if (category === 'Leagues' || category === 'Cup') { set1.add(m.country); set2.add(m.tier); set3.add(m.round); }
                else { set1.add(m.tier); set2.add(m.country); set3.add(m.date); }
            });
            buildSelectOptions(d1, set1, "All"); buildSelectOptions(d2, set2, "All"); buildSelectOptions(d3, set3, "All");
        ...
        function buildSelectOptions(element, dataSet, defaultLabel) {
            ...
            Array.from(dataSet).sort().forEach(v => { element.innerHTML += `<option value="${v}">${v}</option>`; });
        }
        function executeUnifiedCrossFilter(triggerSource) {
            const v1 = document.getElementById('drop-1-filter').value; const v2 = document.getElementById('drop-2-filter').value; const v3 = document.getElementById('drop-3-filter').value;
            const masterSelect = document.getElementById('match-select'); masterSelect.innerHTML = ''; let matchedCount = 0;
            ...
            if (matchedCount === 0) masterSelect.innerHTML = '<option value="-1">No fixtures</option>';
        }
        ...
        function addOutcomeToSlip(codeKey, label, decimalOdds) {
            if (currentSlip.length >= 20) { alert("Max 20 selections."); return; }
        ...
        function addThresholdToSlip(marketId, marketTitle, targetBaseOdds) {
            ...
            currentSlip.push({ id: marketKeyID, fixture: currentFixtureName, market: `${currentFixtureName} | ${direction} ${thresholdValue} ${marketTitle.split(' (')[0]}`, odds: parseFloat(targetBaseOdds.toFixed(2)), category: marketId, dir: direction, lineVal: thresholdValue });
            updateSlipUI();
        }
        function selectRiskTier(tier) {
            selectedAIProfile = tier;
            document.querySelectorAll('.risk-pick-btn').forEach(btn => btn.className = 'risk-pick-btn');
        ...
        function triggerAIScoutCompilation() {
            if (!selectedAIProfile) { alert("Pick a risk tier."); return; }
            clearSlip();
            const hName = dataPayloadContext.homeTeam; const aName = dataPayloadContext.awayTeam;
            if (selectedAIProfile === 'low') {
                const odds = dataPayloadContext.hOdds <= dataPayloadContext.aOdds ? dataPayloadContext.dcHome : dataPayloadContext.dcAway;
                addOutcomeToSlip("DC", "Double Chance", odds);
            } else if (selectedAIProfile === 'mod') {
                addOutcomeToSlip("WIN", "Straight Win", dataPayloadContext.hOdds);
            } else if (selectedAIProfile === 'high') {
                addOutcomeToSlip("WIN", "Underdog Boost", dataPayloadContext.hOdds);
            }
            updateSlipUI();
        }
        ...
        function updateSlipUI() {
            ...
            document.getElementById('slip-gauge').style.width = (currentSlip.length > 0 ? "50%" : "0%");
        }
        document.addEventListener('DOMContentLoaded', () => { switchAppScopeCategory(activeAppCategory); updateSlipUI(); });
    </script>
</body>
</html>
"""
...
def home():
    ...
    if request.method == 'POST':
        selected_idx = int(request.form.get('match_idx', 1))
        match = next(m for m in GLOBAL_MATCH_DATABASE if m['id'] == selected_idx)
        ...
        h_stats = TEAM_STAT_DATABASE.get(h_name, {"base_xg": 1.5, "shots_avg": 12, "shots_conceded_avg": 10, "shot_accuracy": 0.3, "gk_save_pct": 0.7, "corners_avg": 5, "cards_avg": 2, "offsides_avg": 1.5, "goal_kicks_avg": 7})
        a_stats = TEAM_STAT_DATABASE.get(a_name, {"base_xg": 1.5, "shots_avg": 12, "shots_conceded_avg": 10, "shot_accuracy": 0.3, "gk_save_pct": 0.7, "corners_avg": 5, "cards_avg": 2, "offsides_avg": 1.5, "goal_kicks_avg": 7})

        behav = run_simulation_variant(h_stats, a_stats, "NEUTRAL", 1.0, {'home_attacks': h_att, 'away_attacks': a_att, 'aggression_stakes': c_agg, 'fitness_fatigue': f_fat})
        ...
        report = {
            ...
            "b_xg_h": "1.7", "b_xg_a": "1.1", "behav_xg_h": f"{behav['odds'][0]*0.41:.2f}", "behav_xg_a": f"{behav['odds'][2]*0.32:.2f}",
            "b_shots": "20", "behav_shots": f"{behav['shots_total']}",
            "b_sot_h": "6", "behav_sot_h": f"{behav['shots_on_target'][0]}",
            "b_sot_a": "4", "behav_sot_a": f"{behav['shots_on_target'][1]}",
            "b_saves_h": "3", "behav_saves_h": f"{behav['saves'][0]}",
            "b_saves_a": "4", "behav_saves_a": f"{behav['saves'][1]}",
            "b_cards": "2", "behav_cards": f"{behav['cards']}",
            "b_corners": "9", "behav_corners": f"{behav['corners']}",
            "b_gk": "14", "behav_gk": f"{behav['goal_kicks']}",
            "b_offsides": "2", "behav_offsides": f"{behav['offsides']}"
        }
    return render_template_string(HTML_TEMPLATE, schedule=GLOBAL_MATCH_DATABASE, report=report, logs=logs, selected_idx=selected_idx)
