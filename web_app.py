...
# =====================================================================
# 1. EXTENDED TOURNAMENT CORE REGISTRY MATRIX
# =====================================================================
TEAM_STAT_DATABASE = {
...
# Extract unique sorting lists for our navigation controls mapping
...
ALL_DATES = sorted(list(set(m["date"] for m in TOURNAMENT_SCHEDULE)))

# =====================================================================
# 2. CONTEXTUAL LAYERS & MATHEMATICAL ODDS ENGINES
# =====================================================================
def harvest_live_sports_wire(home_team, away_team):
    ...
    if len(scraped_text_blob) > 0:
        ...
        if "fatigue" in scraped_text_blob or "tired" in scraped_text_blob or "rested" in scraped_text_blob:
            f_fat = 0.90
            discovery_logs.append("🏃‍♂️ LIVE WIRE: Squad fatigue or rotation alerts detected. (-10% Accuracy)")
        if "injury" in scraped_text_blob or "injured" in scraped_text_blob or "doubt" in scraped_text_blob:
            h_att *= 0.95; a_att *= 0.95
            discovery_logs.append("🏥 LIVE WIRE: Injury updates or squad fitness checks logged. (-5% Efficiency)")
...
# =====================================================================
# 3. HIGH-END APPLE-INSPIRED INTERACTIVE UI (HTML/CSS/JS)
# =====================================================================
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang='en'>
<head>
    ...
    <style>
        ...
        /* The Three Multi-Dimensional Dropdowns Rows Layout Styling */
        ...
        .filter-menu-box label { font-size: 11px; font-weight: 600; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 4px; letter-spacing: 0.3px; }
        ...
        .slip-add-btn { flex: 1; background: #1c1c1e; border: 1px solid var(--border-card); color: var(--accent-green); padding: 10px; font-size: 12px; font-weight: 700; border-radius: 8px; cursor: pointer; }

        /* Persistent Slip Drawer Component */
        .bet-slip-drawer {
...
<body>
    ...
    <div class='card'>
        <h3>Select Targeted Fixture Pipeline</h3>
    ...
    {% if report %}
        <div class='card'>
            ...
            {% else %}
                <div class='log-line' style='color: var(--text-secondary);'>✅ Clean Wire: No operational adjustments or line variances logged.</div>
            {% endfor %}
        </div>
    ...
    <script>
        ...
        // RE-ENGINEERED: Multi-dimensional conditional filtering cross-matrix solver
        function executeFilter(activeTrigger) {
            ...
            // Reset opposing fields if a single-focus drill-down strategy is chosen
...
def home():
    ...
    if request.method == 'POST':
        ...
        if h_name in TRUE_HOST_NATIONS and h_name.lower().strip() == country.lower().strip():
            ...
            logs.append(f"🏟️ HOST GROUND ACCREDITATION: {h_name} verified on native soil. (+12% Performance Imbalance Encoded)")
        else:
